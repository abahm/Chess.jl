...
"MEMORY EFFICIENT Generate all legal moves on the board, optionally only attacking moves (no castling)"
function generate_moves_new(b::Board; only_attacking_moves=false)
    my_color = b.side_to_move
    enemy_color = opposite_color(my_color)

    move_n = 1
    #moves = b.game_movelist.moves[move_n,ply_n]
