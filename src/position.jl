...
"Called only by generate_moves to check if move can be added to list"
@inline function add_move!(movelist::Movelist, b::Board,
                           my_color::UInt8, my_piece::UInt8,
                           src_sqr::UInt64, dest_sqr::UInt64;
                           promotion_to::UInt8=NONE,
                           en_passant_sqr::UInt64=UInt64(0))
    ...
    # move is a capturing move
    if o!=NONE
        moves[move_index] = Move(my_color, my_piece, src_sqr, dest_sqr, piece_taken=piece_type_on_sqr(b,dest_sqr), promotion_to=promotion_to)
        return BLOCKED  # CAPTURE
    end

    # move to an empty square
    moves[move_index] = Move(my_color, my_piece, src_sqr, dest_sqr, promotion_to=promotion_to, sqr_ep=en_passant_sqr)
